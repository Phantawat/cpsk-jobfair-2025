...
// Lazy load CompanyModal - only loads when user clicks a company
const CompanyModal = dynamic(
  () =>
    import("@/components/companies/CompanyModal").then(
      (mod) => mod.CompanyModal,
    ),
  { ssr: false },
);
...
export default function CompaniesPage({ companies }: CompaniesPageProps) {
  ...
  // Open modal if company query parameter is present
  useEffect(() => {
    ...
    if (companyName && companies.length > 0) {
      ...
      if (company) {
        const index = sortedCompanies.findIndex((c) => c.name === company.name);
        setActiveCompany(company);
        setActiveListIndex(index >= 0 ? index : 0);

        // Clear the query parameter from URL
        router.replace("/companies", undefined, { shallow: true });
      }
    }
  }, [router.query.company, companies, sortedCompanies, router]);
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Browse and filter participating companies at the KU Computer Engineering Job Fair 2025. Find your ideal job opportunity."
        />
        ...
        <header className="bg-gradient-to-r from-ku-pine to-ku-pine-dark shadow-lg sticky top-0 z-20">
          <div className="max-w-[1600px] mx-auto px-6 md:px-12 py-6">
            <div className="flex items-center justify-between">
              ...
              <Link
                href="/"
                className="flex items-center gap-2 text-white hover:text-ku-fresh transition-colors group"
              >
                <svg
              ...
              <div className="text-center flex-1">
                ...
                <p className="text-ku-fresh text-sm md:text-base font-medium">
                  2025
                </p>
              </div>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-4 justify-center">
              ...
              <div className="relative">
                ...
                />
                <svg
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ku-pine pointer-events-none"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div className="relative">
                <select
                  ...
                  value=""
                  onChange={(e) =>
                    e.target.value && handleEmploymentTypeChange(e.target.value)
                  }
                >
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) =>
                    e.target.value &&
                    handleYearLevelChange(Number(e.target.value))
                  }
                >
            ...
            {(filters.selectedEmploymentTypes.length > 0 ||
              filters.selectedYearLevels.length > 0) && (
              <div className="mt-6 flex flex-wrap items-center gap-2 justify-center">
                {filters.selectedEmploymentTypes.map((type) => (
                  <span
                    key={type}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-ku-pine text-white text-xs rounded-lg shadow-sm"
                  >
                    {type}
                    <button
                      onClick={() => handleEmploymentTypeChange(type)}
                      className="hover:text-ku-fresh transition-colors"
                    >
                      <svg
                ...
                {filters.selectedYearLevels.map((year) => (
                  ...
                  >
                    Year {year}
                    <button
                      onClick={() => handleYearLevelChange(year)}
                      className="hover:text-ku-fresh transition-colors"
                    >
                      <svg
                        className="w-3.5 h-3.5"
                ...
                <button
                  ...
                  className="text-xs text-ku-pine hover:text-ku-pine-dark transition-colors underline font-semibold"
                >
                  Clear all
                </button>
              </div>
          ...
          {sortedCompanies.length > 0 ? (
            <CompanyList
              ...
              onSelectCompany={(company, index) =>
                handleSelectCompany(company, index)
              }
              onKeyDown={handleKeyboardNavigation}
            />
          ) : (
