...
export const CompanyModal = React.memo<CompanyModalProps>(
  ({ company, onClose }) => {
    ...
    return (
      <div
        className="fixed inset-0 bg-ku-pine/40 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-6 z-50 animate-fadeIn glass-dark"
        onClick={onClose}
        role="presentation"
      >
        <div
          ref={modalRef}
          className="bg-transparent rounded-t-2xl md:rounded-2xl max-w-2xl w-full max-h-[90vh] shadow-2xl animate-slideDown md:animate-fadeInUp"
        ...
        >
          ...
          >
            {/* Front of Card */}
            <div
              className="bg-white/95 backdrop-blur-md rounded-t-2xl md:rounded-2xl w-full max-h-[90vh] overflow-y-auto glass-light"
              style={{
            ...
            >
              ...
              <div className="sticky top-0 bg-white border-b border-gray-200 px-5 py-4 z-10">
                ...
                <div className="flex flex-col items-center text-center pt-2">
                  ...
                  <h2
                    id="company-name"
                    className="text-xl md:text-2xl font-bold text-gray-900 mb-1 leading-tight"
                  >
                    {company.name}
                  </h2>
                  <p
              ...
              <div className="px-6 py-4 space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h3 className="text-sm font-semibold text-gray-800 mb-2 flex items-center gap-2">
                    <svg
                      className="w-4 h-4 text-ku-pine"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5.581m0 0H9m11.581 0a2 2 0 100-4H9m11.581 4a2 2 0 100-4H9m0 16V9m0 0a2 2 0 10-4 0m4 0a2 2 0 11-4 0m15-6h-2m0 0h-2m2 0a2 2 0 100-4m0 4a2 2 0 11-4 0m4 0V5m-4 4H7"
                      />
                    </svg>
                    Business Type
                  </h3>
                ...
                <div className="flex items-center gap-2 text-sm bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                  <svg
                    className="w-4 h-4 text-ku-pine flex-shrink-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span className="text-gray-800">
                    <span className="font-semibold">Participation Time:</span>{" "}
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {company.positions.map((pos: string, i: number) => {
                      const colors = [
                      ...
                      return (
                        <span
                          key={i}
                          className={`inline-flex items-center text-xs px-3 py-1.5 rounded-md font-medium ${colorClass}`}
                        >
                          {pos}
                        </span>
                      );
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-sm font-semibold text-gray-600 mb-2 flex items-center gap-1.5">
                      <svg
                        className="w-3.5 h-3.5 text-ku-pine"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Array.from(
                        new Set(
                          company.employmentTypes.map((type: string) => {
                            if (
                              type.includes("full-time") ||
                              type.includes("เต็มเวลา")
                            ) {
                              return "Full-time";
                            } else if (
                              type.includes("part-time") ||
                              type.includes("ไม่เต็มเวลา")
                            ) {
                              return "Part-time";
                            } else if (
                              type.includes("intern") ||
                              type.includes("ฝึกงาน")
                            ) {
                              return "Internship";
                            }
                            return type;
                        ...
                        <span
                          key={i}
                          className="px-3 py-1 bg-ku-fresh text-ku-pine-dark text-xs font-medium rounded-md"
                        >
                          {displayType}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {company.yearLevels.length > 0 ? (
                        company.yearLevels.map((year: number) => (
                          <span
                            key={year}
                            className="px-3 py-1 bg-ku-fresh text-ku-pine-dark text-xs font-semibold rounded-md"
                          >
                            Year {year}
                          </span>
                        ))
